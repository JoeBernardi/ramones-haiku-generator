function new_count(word) {
    ...
    if (word === "d.u.m.b" || word === "1-2-3-4" || word === "4-5-6-7") { numberOfSyllables = 4; }
    else if (word === "ph.d" || word === "SLA" || word === "pt-boat" || word === "somebody" || word === "lsd" || word === "lsd," || word === "ddt" || word === "cia" || word === "ooo-ooo-whee!" || word === "53rd" || word === "everything's") { numberOfSyllables = 3; }
    else if (word === "tryin'" || word === "moonlight?" || word === "lovely" || word === "wanted" || word === "couldn't" || word === "basement" || word === "real" || word === "somethin'" || word === "daughter?" || word === "going" || word === "viet" || word === "lying" || word === "tv's" || word === "doing" || word === "gimme" || word === "tired" || word === "cryin'" || word === "schatze") { numberOfSyllables = 2; }
    else if (word.length <= 3 || word === "hand?" || word === "there's" || word === "where's" || word === "future's" || word === "rule") { numberOfSyllables = 1; }
      ...
      //if there's not a question mark in it we're good, otherwise it generally guesses one syllable too low. don't go below zero, though.
            if (word.indexOf('?') === -1) {
                numberOfSyllables = word.match(/[aeiouy]{1,2}/g).length;
            }
        ...
        if (word.match(/[aeiouy]{1,2}/g).length-1 === 0)
        {
          ...
          numberOfSyllables = word.match(/[aeiouy]{1,2}/g).length-1;
        }
      }
    }
  ...
  function rollinHaiku(data) {
    ...
    while (syllableCount < 17)
    {
      ...
      syllables = new_count(data[randomIndex]);
    ...
    if(syllableCount > 17 || hitFive === false || hitTwelve === false) {
      returnedHaiku =  rollinHaiku(data);
    }
    else {
      returnedHaiku = haiku;
    }
    return returnedHaiku;
  }
  ...
  function getSyllableLines() {
    ...
    $.get('lyrics.txt', function(data) {
      ...
      var haiku = rollinHaiku(data),
        firstLine = haiku[0].toString().replace(/,/g, " "),
        secondLine = haiku[1].toString().replace(/,/g, " "),
        thirdLine = haiku[2].toString().replace(/,/g, " ");
...
function getRegularLines() {
  ...
  $.get('lyrics.txt', function(data) {
    ...
    data = data.split("\n");
    var lines = [], i = 0, lineNumber;
    while (i < 3) {
      lineNumber = Math.floor(Math.random() * data.length);
